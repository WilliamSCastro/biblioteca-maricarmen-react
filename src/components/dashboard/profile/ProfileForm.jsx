...
export function ProfileForm({ user, onUserUpdate }) {
  ...
  async function handleSubmit(e) {
    ...
    if (file && file.name !== "" && !isValidFileType(file)) {
      newErrors.avatar = "Format de imatge invàlid. Només poden ser JPG, JPEG i PNG.";
    }
    ...
    if (res.success) {
      if (res.data.type === RESPONSE_TYPES.SUCCESS_MODIFY) {
      ...
      if (res.type === ERROR_TYPES.ERROR_AT_DATA) {
